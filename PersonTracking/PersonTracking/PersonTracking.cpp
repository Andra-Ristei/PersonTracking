...
void reduce_distance_to_person(t_wheels_controller ctrl) {
	...
	if (new_distance > ctrl.get_minimum_distance_allowed()) {
...
void increase_distance_to_person(t_wheels_controller ctrl) {
	double new_distance = ctrl.get_distance_to_person() + ctrl.get_modify_distance_by();
	if (new_distance < ctrl.get_maximum_distance_allowed()) {
		ctrl.set_distance_to_person(new_distance);
	}
}
...
int read_sensor(int sensor, t_jenny5_command_module head_controller) {
	int distance = 0;;
...
void move_closer(int wait_for, t_wheels_controller wheels_ctrl, t_jenny5_command_module head_ctrl) {
	...
	while ((current_distance = read_sensor(ULTRASONIC, head_ctrl)) > wheels_ctrl.get_distance_to_person()) {
...
int main() {
	...
	while (capture.read(frame)) {
		...
		if (frame_number == 10) {
			...
			face_tracking.detect_faces_and_display(frame);
			...
			int current_distance = read_sensor(ULTRASONIC, command_module);
